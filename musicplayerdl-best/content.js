...
document.addEventListener(
  ...
  function (event) {
    ...
    // Store the URL for later use - not actually needed but keeping for consistency
    if (videoUrl) {
...
function enhanceYouTubePlayer() {
  ...
  if (!document.getElementById("ytm-best-download-button")) {
    const menuContainer = document.querySelector(".ytp-right-controls");
    if (menuContainer) {
      const downloadButton = document.createElement("button");
      downloadButton.id = "ytm-best-download-button";
      downloadButton.className = "ytp-button";
      downloadButton.title = "Download Best Quality with MusicPlayerDL";
      downloadButton.innerHTML = "↓"; // Simple download icon

      // Style the button
      downloadButton.style.fontSize = "16px";
      downloadButton.style.fontWeight = "bold";
      downloadButton.style.color = "#ff0000"; // Red color to distinguish it from other buttons
...
document.addEventListener(
  ...
  function (event) {
    ...
    if (videoElement) {
      videoElement.setAttribute("data-musicplayerdl-best-target", "true");
    }
  },
  true
);
